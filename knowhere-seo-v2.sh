...
def write_sitemap():
    ...
    # KW:CONCEPT 17 Sep 2026 — the concept pages (/hsc/..., /vce/...) are generated by build-concept-pages.mjs
    # and are NOT in PAGES. Without this block every SEO regen silently dropped them out of the sitemap.
    # Rebuilt from disk each run, so the two generators can never revert each other.
    for cert in ("hsc", "vce"):
        base = os.path.join(ROOT, cert)
