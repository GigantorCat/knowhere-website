...
/* send-launch.js — the three countdown emails, per cohort, straight from the Waitlist contacts.
   ...
   Stages: t7 (one week out) · t3 (three days) · t0 (launch day, needs --app https://app.knowhere.me) · t4 (Fri 25 Sep "send this to your parent" — students only, other cohorts skipped)
...
if (!emails.STAGES.includes(stage)) { console.error('usage: node send-launch.js t7|t3|t0|t4 --date "Monday 21 September" [--app URL] [--test you@x.com --as student|parent|teacher] [--send] [--only student|parent|teacher]'); process.exit(1); }
...
(async () => {
  if (opt('--test')) {
    ...
    const m = emails.build(stage, { email: opt('--test'), firstName: 'Cat', type, teacherSeat: type === 'teacher' ? 7 : 0, launchDateStr, appUrl });
    if (!m) { console.log(`no ${stage} email goes to ${type}s — nothing to test`); return; }
    const r = await resend('POST', '/emails', m);
    console.log(r.ok ? `test sent → ${opt('--test')} (${type})` : `FAILED ${r.status} ${JSON.stringify(r.json)}`); return;
  }
  await seedStats();
  const list = contacts.filter(c => !c.unsubscribed && (!only || (c.properties || {}).user_type === only) && emails.build(stage, { email: c.email, firstName: c.first_name, type: TYPE((c.properties || {}).user_type), launchDateStr, appUrl }));
  ...
  for (let i = 0; i < list.length; i += 100) {
    const batch = list.slice(i, i + 100).map(c => {
      ...
      const m = emails.build(stage, { email: c.email, firstName: c.first_name, type: TYPE(p.user_type), teacherSeat: Number(p.founding_teacher_seat) || 0, launchDateStr, appUrl });
      return { from: m.from, reply_to: m.reply_to, to: [m.to], subject: m.subject, html: m.html, text: m.text, headers: m.headers, tags: [{ name: 'stage', value: stage }, { name: 'cohort', value: TYPE(p.user_type) }] };
    });
